...
function TrialWidget({ business }) {
  ...
  return (
    <div className="relative" ref={ref}>
      <button
        onClick={() => setOpen(o => !o)}
        className={`flex items-center gap-1.5 px-3 py-1.5 rounded-xl border text-xs font-medium transition-colors min-h-[36px] ${color}`}
      >
        <HiOutlineClock className="w-3.5 h-3.5 flex-shrink-0" />
        <span className="hidden sm:block">باقي {daysLeft > 0 ? daysLeft : 0} يوم في تجربتك المجانية</span>
        <span className="sm:hidden">{daysLeft > 0 ? daysLeft : 0} يوم</span>
      </button>
...
export default function TopBar({ title }) {
  ...
  return (
    <header className="h-14 bg-white border-b border-gray-100 flex items-center px-4 sm:px-6 gap-3" dir="rtl">
      ...
      <div className="flex items-center gap-2">
        ...
        {isMultiBranch && currentBranch && (
          <div className="relative" ref={dropRef}>
            <button
              onClick={() => setOpen(o => !o)}
              className="flex items-center gap-1.5 px-3 py-1.5 rounded-xl bg-slate-50 border border-slate-200 text-sm text-slate-700 hover:bg-slate-100 transition-colors min-h-[36px]"
            >
              {currentBranch.isAll ? (
        ...
        {business && (
          <div className="flex items-center gap-2">
            <div className="w-7 h-7 bg-accent-100 rounded-full flex items-center justify-center text-accent-700 text-xs font-bold flex-shrink-0">
