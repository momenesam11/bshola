...
export default function ScheduleBlockEditor({ value, onChange }) {
  const blocks = value || {}
              ...
              {dBlocks.map((b, idx) => (
                <div key={idx} className="flex items-center gap-2 flex-wrap">
                  <input type="time" value={b.start} onChange={e => updateBlock(key, idx, 'start', e.target.value)}
                    className="border border-slate-200 rounded-lg px-2 py-1.5 text-sm focus:outline-none focus:ring-2 focus:ring-accent-400" dir="ltr" />
